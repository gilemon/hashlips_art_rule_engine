# Welcome to HashLips with rules 👄
...
If you are using a platform not supported by the prebuilds of the canvas module (e.g. Mac M1 or Raspberry), you can install it from sources with:
...
This modified version introduce rules that can be used for avoiding to create final images with inconsistent traits. For instance a monster with different bottom and top skin colors.

The `cantStartRule: true` is automatically set to `false` for the last layer in order to avoid it to start a rule that can't be propagated as it's the last layer. If you really want to enable it you need to set it to `true` as an option.

#### Level

The level rule allows to save layer images inside subfolders from the main layer directory. For example if a layer image in the `Bottom lid/green` subdirectory is chosen, the rule engine will try to find a subdirectory with the same level (name) in the following layer directories; in this case `Top lid/green`.
